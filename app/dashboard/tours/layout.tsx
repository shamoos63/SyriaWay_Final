...
function TourGuideSidebar({ activeTab }: { activeTab: string }) {
  const { user } = useAuth()

  const navigationItems = [
    {
      name: "Dashboard",
      href: "/dashboard/tours?tab=dashboard",
      icon: Compass,
      active: activeTab === "dashboard"
    },
    {
      name: "My Tours",
      href: "/dashboard/tours?tab=tours",
      icon: MapPin,
      active: activeTab === "tours"
    },
    {
      name: "Bookings",
      href: "/dashboard/tours?tab=bookings",
      icon: Calendar,
      active: activeTab === "bookings"
    },
    {
      name: "Special Requests",
      href: "/dashboard/tours?tab=requests",
      icon: Users,
      active: activeTab === "requests"
    },
    {
      name: "Customers",
      href: "/dashboard/tours?tab=customers",
      icon: Users,
      active: activeTab === "customers"
    },
    {
      name: "Earnings",
      href: "/dashboard/tours?tab=earnings",
      icon: DollarSign,
      active: activeTab === "earnings"
    },
    {
      name: "Settings",
      href: "/dashboard/tours?tab=settings",
      icon: Settings,
    ...
    <div className="flex flex-col h-full bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 border-r border-slate-200 dark:border-slate-700">
      ...
      <div className="p-6 border-b border-slate-200 dark:border-slate-700">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-white">Tour Guide</h1>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <Badge variant="secondary" className="mt-1 text-xs">
              {user?.role?.replace('_', ' ') || "Tour Guide"}
            </Badge>
          </div>
        </div>
      </div>
...
export default function TourGuideDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div dir={dir} className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
      <div className="flex min-h-screen">
        ...
        >
          <TourGuideSidebar activeTab={activeTab} />
        </div>
            ...
            <div className="flex items-center justify-between px-6 py-4">
              <div className="flex items-center space-x-4">
                ...
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
                  ...
                  {activeTab === "tours" && "My Tours"}
                  {activeTab === "bookings" && "Bookings"}
                  {activeTab === "requests" && "Special Requests"}
