...
import { Menu, X, Hotel, BarChart3, Settings, Users, Calendar, DollarSign, Home, LogOut } from "lucide-react"
...
function HotelOwnerSidebar({ activeTab }: { activeTab: string }) {
  const { user } = useAuth()
  ...
  const navigationItems = [
    {
      name: "Overview",
      href: "/dashboard/hotels?tab=overview",
      icon: BarChart3,
      active: activeTab === "overview"
    },
    {
      name: "Room Management",
      href: "/dashboard/hotels?tab=rooms",
      icon: Hotel,
      active: activeTab === "rooms"
    },
    {
      name: "Bookings",
      href: "/dashboard/hotels?tab=bookings",
      icon: Calendar,
      active: activeTab === "bookings"
    },
    {
      name: "Customers",
      href: "/dashboard/hotels?tab=customers",
      icon: Users,
      active: activeTab === "customers"
    },
    {
      name: "Earnings",
      href: "/dashboard/hotels?tab=earnings",
      icon: DollarSign,
      active: activeTab === "earnings"
    },
    {
      name: "Settings",
      href: "/dashboard/hotels?tab=settings",
      icon: Settings,
    ...
    <div className="flex flex-col h-full bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 border-r border-slate-200 dark:border-slate-700">
      ...
      <div className="p-6 border-b border-slate-200 dark:border-slate-700">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-white">Hotel Management</h1>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <Badge variant="secondary" className="mt-1 text-xs">
              {user?.role?.replace('_', ' ') || "Hotel Owner"}
            </Badge>
          </div>
        </div>
      </div>
...
export default function HotelOwnerDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div dir={dir} className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
      <div className="flex min-h-screen">
        ...
        >
          <HotelOwnerSidebar activeTab={activeTab} />
        </div>
